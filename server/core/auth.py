...
def save_device(db: Session, user_id: int, device_id: str, device_data: Dict[str, Any]) -> Device:
    ...
    if user and device_count >= user.devices_limit:
        raise HTTPException(
            ...
            detail=f"Лимит устройств: максимум {user.devices_limit}"
        )
    ...
    # Проверка на существующее устройство
...
def delete_device(db: Session, api_token: str, device_token: str) -> Tuple[bool, str]:
    """Удаление устройства"""
    ...
    if not device:
        return False, "Устройство не найдено или доступ запрещён"

    # Удаление всех сообщений устройства
    db.query(Message).filter(Message.device_token == device_token).delete()
    db.delete(device)
    db.commit()

    return True, f"Устройство {device.device_id} удалено"